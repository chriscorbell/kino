...
Community intro markers require one known release version within a minute of the video's runtime ([ADR 0029](docs/adr/0029-match-community-intros-by-release-version.md)). `pnpm intro:check` exercises the bundled client against HTTP fixtures shaped as the live service answers, `pnpm macos:check-intro` repeats the match and cancellation checks in Qt WebEngine, `PlayerScreen.intro.test.tsx` drives the desktop player's Skip Intro, automatic skip and Undo, and `pnpm android:check` drives the real Media3 player and remote on the Shield. `pnpm intro:check-live` runs the production client against TheIntroDB itself; it needs the network, so it runs by hand.

### Linux and Windows shells

The same shell builds on Linux and Windows. On Ubuntu 26.04, which is where Qt 6.8 and libmpv 2.4 first meet, install the build dependencies, then build and run the shell from the repository root. `qt6-wayland` lets it draw as a Wayland client rather than through XWayland. Hardware decoding needs the GPU's VA-API driver: Ubuntu installs AMD's, but not Intel's `intel-media-va-driver`, and without a driver Kino refuses every video rather than decode it in software. The Flatpak brings its own drivers.
...
The Flatpak manifest, `packaging/flatpak/com.chriscorbell.Kino.yml`, builds libmpv with FFmpeg, libplacebo and VA-API, the torrent engine, and the shell from pinned sources on the KDE runtime and the Qt WebEngine base app. `.github/workflows/flatpak.yml` builds it, starts it on a virtual display, checks that a second launch reaches the first, and uploads the bundle; a second job builds the engine natively from the manifest's libtorrent and Boost pins and runs its tests and the embedded-profile check, which the Flatpak build does not. The build sandbox has no network for pnpm, so the interface is built first: with `flatpak-builder` installed, run `pnpm --filter @kino/desktop build`, then `flatpak-builder --user --install build/flatpak packaging/flatpak/com.chriscorbell.Kino.yml`. The Ubuntu build and the Flatpak also run the probes and the playback gate on Linux hardware; [the Linux validation record](docs/validation/linux-hardware.md) has the last runs and how to repeat them.

Windows builds with MSVC against Qt 6.11 with Qt WebEngine and libmpv. `scripts/build-windows-mpv.sh` cross-compiles libmpv on Linux with MinGW-w64 from pinned sources, the Flatpak's where the two share a library, into one `libmpv-2.dll` that needs nothing but Windows. CMake takes the directory holding libmpv's `include` folder and an MSVC `mpv.lib` as `-DKINO_MPV_DIR`; the MinGW build makes only a MinGW import library, so CI makes `mpv.lib` from the DLL's exports. `.github/workflows/windows.yml` is the exact recipe. It runs the unit tests, builds a portable folder with the Qt libraries, the C++ runtime, and `libmpv-2.dll` beside `Kino.exe` that runs from any folder, and runs the launch, navigation, focus and scale probes against it. A second job builds the torrent engine with vcpkg, and the package job adds it and the license notices to the `Kino-windows-x64-with-engine` zip. The probes and the playback gate also run on Windows hardware; [the Windows validation record](docs/validation/windows-hardware.md) has the last run and how to repeat it.
