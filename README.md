...
The native Kotlin/Compose development app runs on the NVIDIA Shield. It includes remote navigation, Stremio device-link sign-in, browsing, details, source selection, and hardware SDR playback. See [Android TV development](docs/ANDROID-TV.md) for toolchain setup, device checks, and current limitations.
...
Native playback uses the Stremio profile's preferred audio language when selecting a file's audio track. The Audio tracks picker lists embedded tracks by language, title, and codec, reports the selected track, and allows switching without reloading the source. A manual choice applies to the current file; the next load uses the preference again, with mpv's default when that language is unavailable. `pnpm macos:check-audio` generates a video with English and Spanish audio and verifies preferred selection, manual switching, the next load, and fallback through production QML and WebChannel. The existing three-argument `load` bridge remains available; current Kino uses `loadWithAudioLanguage`.
...
Settings exposes device-local seeding and download limits under Torrent streaming. Seeding defaults to enabled and the rate cap defaults to Unlimited. Rate options use MiB/s; existing non-preset limits remain visible. Each change updates the active helper, saves its configuration, and reads the accepted values back before showing success. Failed reads and writes offer Retry. These settings survive app restart and cache clearing.
