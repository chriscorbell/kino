...
describe('default resume', () => {
  it('selects the saved episode source without playback-start controls', async () => {
    const saved = {
      ...details('ep2'),
      libraryItem: { id: meta.id, timeOffset: 30_000, videoId: 'ep2' },
    };
    const { onPlay } = mountDetails(saved, meta, 'ep2');
    const source = await screen.findByRole('button', { name: /ep2 source/ });
