...
Desktop uses libmpv through the initial shell fork. Android TV uses Media3 with Kino's own HDR10-to-SDR shader, because Media3's tone mapping cannot run on the Shield ([ADR 0021](adr/0021-tone-map-hdr-in-kino-rather-than-media3.md)). On TV, HDR10 is accepted, while HLG and Dolby Vision are rejected before a decoder is configured until each is measured on the device the way HDR10 was.
