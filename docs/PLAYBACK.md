...
On macOS, mpv tone-maps HDR10 and Dolby Vision profile 8's HDR10 base layer, and the `pixels-` cases in `pnpm macos:check-playback` read the drawn frame of a probe whose every pixel is a known code word. Its neutral ramp must match the host reference in `scripts/test-support/tone-map-reference.mjs` within 0.03 and its in-gamut colours within 0.05; dropping the BT.2020 to BT.709 conversion misses by about 0.14. An SDR ramp in the same layout runs first as a control, proving a drawn frame can be read back at all. GitHub's hosted Mac runners read every frame as black, so CI sets `KINO_PLAYBACK_PIXELS=optional`: there a failed control reports the pixel cases as not run, and anywhere else it fails them. The pixel cases are therefore checked on a Mac with a real GPU, which `pnpm macos:check-playback` does by default. Profile 5 has no base layer this renderer can show without Dolby's reshaping, so the player refuses it by the profile mpv reports, and the `dv-p5-probe` case checks the refusal.
