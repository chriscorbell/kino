...
Later Homebrew releases are carried forward with `pnpm notices:refresh` when every retained text is unchanged, as `third_party/notices/README.md` describes; `third_party/notices/reviewed.json` records the release currently reviewed.
