...
On 2026-09-26 Kino ran the desktop probes and the playback gate on two Linux machines, both built from `main` with the change in #257, in two forms: the shell built on Ubuntu 26.04 by the steps in the README, and the `Kino-x86_64.flatpak` bundle from the Flatpak workflow.
...
The Ubuntu build passed all 26 playback fixtures on both machines, decoding through VA-API: H.264, HEVC SDR, HDR10 at 1080p and 2160p, HLG, and AV1; FFV1 rejected as software-only; ALAC, DTS, PCM, E-AC-3 and AC-3 audio; embedded, authored and external subtitles; chapters; the pause before sleep; Stereo loudness normalization; drawn HDR10 and Dolby Vision 8.1 pixels matching the tone-map reference; Dolby Vision 5 rejected; refresh-rate matching, which leaves the display as it is on Linux; and broken or missing sources rejected.

On the laptop it also passed launch and the second-launch hand-over, scale, focus, resume, community intros, seasons, fullscreen, volume, audio, track choices, the web console, navigation, interface recovery, TLS, request headers, shutdown, and add-on transports.
...
- The interface recovery probe crashed the web process with DevTools' `Page.crash`, which does nothing on Linux. It now kills the process instead (#259), and Linux CI runs it.
...
For the Ubuntu build, follow the README and set `KINO_APP_BINARY` to `build/linux/Kino`. For the Flatpak, install the bundle with `flatpak install --user`, after `org.kde.Platform//6.11` from Flathub, and point `KINO_APP_BINARY` at a wrapper script that runs `flatpak run --filesystem=/tmp --filesystem=home` with `--env=` for each `KINO_*` and `QTWEBENGINE_*` variable. Set `XDG_DATA_HOME` to `~/.var/app/com.chriscorbell.Kino/data` so the web console probe finds the sandbox's log.
