...
This build supports direct HTTPS media, HLS, and DASH through Media3. Torrent sources remain unavailable until the approved engine is ported. HDR sources are still rejected. The conversion itself is solved and gated, as [ADR 0021](adr/0021-tone-map-hdr-in-kino-rather-than-media3.md) records, and connecting it to playback is the remaining step; the libmpv alternative ADR 0003 reserved is not being taken. Dolby Vision and HLG stay rejected until each is measured the way HDR10 was.
