...
A compact icon rail expands into a labeled drawer when focused. The first Home poster receives focus on entry. Left from the first poster returns to the active navigation item, and Back from details restores the selected poster and its scroll position. Search groups movies and series into rows. Library has All, Movies, and Series filters. Add-ons has its own read-only list. Settings provides account actions, Audio output, subtitles on/off, preferred audio and subtitle languages, artwork cache size and clearing, and Copy diagnostic summary.
