...
The TV presentation still needs Discover and pagination, add-on management, complete subtitle controls, audio output settings and passthrough validation, Skip Intro and Up Next, release update handling, and distribution notices. Signed-in browsing and saved-account restoration have been exercised on the Shield. Source playback has reported audio and control issues tracked in [#140](https://github.com/chriscorbell/kino/issues/140) and [#144](https://github.com/chriscorbell/kino/issues/144). This build does not yet meet the Phase 4 daily-driver exit criteria.
