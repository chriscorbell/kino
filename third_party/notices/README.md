...
Run `pnpm notices:check` to verify retained text hashes and dependency pins, including the Android record against `gradle.lockfile`, the TV Core's `Cargo.lock`, and the versions in `scripts/build-android.py`, and the Linux record against the modules and source pins in the Flatpak manifest. Run a clean `pnpm macos:package` to verify the complete staged artifact. The package collector performs no downloads and rejects missing coverage before signing.
